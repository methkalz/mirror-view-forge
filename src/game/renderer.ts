...
export function render(ctx: CanvasRenderingContext2D, g: GameData) {
  ...
  if (g.breakingNews) {
    const bn = g.breakingNews;
    const progress = 1 - bn.timer / bn.duration;
    // Fade in first 0.4s, hold, fade out last 0.5s
    let alpha = 1;
    if (progress < 0.12) alpha = progress / 0.12;
    else if (bn.timer < 0.5) alpha = bn.timer / 0.5;
    ...
    const cx = g.width / 2;
    const bannerY = g.height * 0.38;
    const bannerH = 90;

    // Red breaking news banner bar
    ctx.save();
    ctx.globalAlpha = alpha;

    // Main banner background
    const bannerGrad = ctx.createLinearGradient(0, bannerY, 0, bannerY + bannerH);
    bannerGrad.addColorStop(0, 'rgba(180,20,20,0.95)');
    bannerGrad.addColorStop(0.5, 'rgba(140,10,10,0.98)');
    bannerGrad.addColorStop(1, 'rgba(100,5,5,0.95)');
    ctx.fillStyle = bannerGrad;
    ctx.fillRect(0, bannerY, g.width, bannerH);

    // Gold accent lines (top and bottom)
    ctx.fillStyle = '#fbbf24';
    ctx.fillRect(0, bannerY, g.width, 3);
    ctx.fillRect(0, bannerY + bannerH - 3, g.width, 3);

    // "خبر عاجل" badge (top-left red pill)
    const badgeW = 120;
    const badgeH = 26;
    const badgeX = g.width * 0.08;
    const badgeY = bannerY - badgeH / 2;
    ctx.fillStyle = '#dc2626';
    ctx.beginPath();
    ...
    ctx.textBaseline = 'middle';
    ctx.direction = 'rtl';
    ctx.fillText('خبر عاجل', badgeX + badgeW / 2, badgeY + badgeH / 2);

    // Pulsing red dot (live indicator)
    const dotPulse = 0.5 + Math.sin(g.elapsed * 6) * 0.5;
    ctx.fillStyle = `rgba(255,60,60,${dotPulse})`;
    ctx.beginPath();
    ctx.arc(badgeX - 10, badgeY + badgeH / 2, 4, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = '#ff3c3c';
    ctx.beginPath();
    ctx.arc(badgeX - 10, badgeY + badgeH / 2, 2, 0, Math.PI * 2);
    ctx.fill();
    ...
    ctx.textAlign = 'center';
    ctx.direction = 'rtl';
    ctx.fillText(bn.text, cx, bannerY + 35);

    // Sub-headline
    ctx.fillStyle = 'rgba(255,220,180,0.9)';
    ctx.font = '15px Tajawal, sans-serif';
    ctx.fillText(bn.subText, cx, bannerY + 62);

    // Wave number badge (bottom-right)
    ctx.fillStyle = 'rgba(0,0,0,0.6)';
    ctx.font = 'bold 12px Tajawal, monospace';
    ctx.textAlign = 'right';
    ctx.direction = 'ltr';
    ctx.fillText(`WAVE ${g.waveNumber}`, g.width * 0.92, bannerY + bannerH + 18);

    // Scrolling ticker line at bottom (decorative)
    const tickerY = bannerY + bannerH + 6;
    ctx.fillStyle = 'rgba(220,38,38,0.8)';
    ctx.fillRect(0, tickerY, g.width, 20);
    ctx.fillStyle = '#fbbf24';
    ctx.fillRect(0, tickerY, g.width, 1);
    ctx.fillRect(0, tickerY + 19, g.width, 1);
    // Scrolling dots
    const scrollOffset = (g.elapsed * 80) % g.width;
    ctx.fillStyle = 'rgba(255,255,255,0.5)';
    ctx.font = '10px monospace';
    ctx.textAlign = 'left';
    ctx.direction = 'ltr';
    const tickerText = '★ SKYFALL ALERT ★ SKYFALL ALERT ★ SKYFALL ALERT ★ SKYFALL ALERT ★';
    ctx.fillText(tickerText, -scrollOffset, tickerY + 14);
    ctx.fillText(tickerText, g.width - scrollOffset, tickerY + 14);
